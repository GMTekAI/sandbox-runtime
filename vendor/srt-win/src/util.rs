...
/// borrow the pointer via [`OwnedSd::as_psd`] for use in a
/// `SECURITY_ATTRIBUTES`. Fields are `pub(crate)` so callers that
/// need the byte view (e.g. `wfp.rs`'s `FWP_BYTE_BLOB` provider
/// data) can read them without pulling subsystem-specific types
/// into util.
pub struct OwnedSd {
...
impl OwnedSd {
    ...
    /// Borrow the SD pointer for use in a `SECURITY_ATTRIBUTES`.
    /// Caller must keep `self` alive for the duration.
    pub fn as_psd(&self) -> PSECURITY_DESCRIPTOR {
        self.ptr
    }
}
